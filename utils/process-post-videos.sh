...
# shellcheck source=lib/video-derivatives.sh
source "$SCRIPT_DIR/lib/video-derivatives.sh"
...
usage() {
  ...
  echo "  --renditions LIST  Long-edge sizes in px (default: 720,1080)" >&2
  exit 1
}
...
if [[ -n "$FROM_LIST" ]]; then
  ...
  INPUT_FILE="$(mktemp "${TMPDIR:-/tmp}/post-video-process.XXXXXX")"
  trap 'rm -f "$INPUT_FILE"' EXIT
  cat > "$INPUT_FILE"
fi

if [[ -n "$INPUT_FILE" ]]; then
  vd_filter_hls_masters < "$INPUT_FILE" | vd_process_masters \
    "$MEDIA_ROOT" "$DERIVED_ROOT" "$RENDITIONS" "$FORCE" "$DRY_RUN"
else
